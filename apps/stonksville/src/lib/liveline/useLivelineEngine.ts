...
export function useLivelineEngine(
  ...
  const pausedMultiDataRef = useRef<Map<string, { data: LivelinePoint[]; value: number }> | null>(
    null,
  );
  ...
  const draw = useCallback(() => {
    ...
    if (!cfg.paused && timeDebtRef.current > 0.001) {
      const catchUpSpeed =
        timeDebtRef.current > 10 ? PAUSE_CATCHUP_SPEED_FAST : PAUSE_CATCHUP_SPEED;
    ...
    if (isCandle) {
      ...
      useStash =
        !useMultiStash && !hasData && chartReveal > 0.005 && lastDataRef.current.length >= 2;
    ...
    if (!hasData && !useStash && !useMultiStash) {
      ...
      const loadingColor =
        isCandle || cfg.isMultiSeries || lastMultiSeriesRef.current.length > 0
    ...
    if (isCandle) {
      ...
      const rawLive = pausedCandlesRef.current
        ? (pausedLiveRef.current ?? undefined)
        : cfg.liveCandle;
      ...
      if (rawLive) {
        if (!closeLineSmoothInitedRef.current) {
          ...
          closeLineSmoothRef.current = lerp(
            closeLineSmoothRef.current,
            rawLive.close,
      ...
      if (rawLive) {
        if (!lineSmoothInitedRef.current) {
          ...
          lineSmoothCloseRef.current = lerp(
      ...
      if (effectiveLineValue !== undefined && hasTickData) {
        if (!lineTickSmoothInitedRef.current) {
          ...
          const gapRatio = Math.min(valGap / prevRange, 1);
          const adaptiveSpeed = LINE_LERP_BASE + (1 - gapRatio) * LINE_ADAPTIVE_BOOST;
          lineTickSmoothRef.current = lerp(
      ...
      if (lineModeProg > 0.01 && drawLive && lineSmoothInitedRef.current) {
        const blended =
          drawLive.close + (lineSmoothCloseRef.current - drawLive.close) * lineModeProg;
      ...
      if (
        ...
        lineSmoothValue =
          lineSmoothCloseRef.current + (smoothTick - lineSmoothCloseRef.current) * lineDensityProg;
      } else {
      ...
      drawCandleFrame(ctx, layout, cfg.palette, {
        ...
        liveCandle: drawLive,
        closePriceCandle:
          closeLineSmoothInitedRef.current && rawLive
        ...
        if (lineModeProg > 0.5 && cfg.showBadge) {
          ...
          if (badgeRef.current.container.style.display !== "none") {
            const base = badgeRef.current.container.style.opacity
              ? parseFloat(badgeRef.current.container.style.opacity)
              : 1;
      ...
      const recentDelta =
        lookback > 0
          ? Math.abs(
              visible[visible.length - 1].value - visible[visible.length - 1 - lookback].value,
            )
          : 0;
      ...
      if (badge) {
        ...
        if (pauseProgress > 0.01 && badge.container.style.display !== "none") {
          const base = badge.container.style.opacity
            ? parseFloat(badge.container.style.opacity)
            : 1;
