...
const divisible = (val: number, interval: number): boolean => {
  const ratio = val / interval;
  return Math.abs(ratio - Math.round(ratio)) < 0.01;
};

/** Fine labels hide when cramped (<40px) and fade in with space (60px+). */
const fineLabelTarget = (finePx: number): number => {
  if (finePx < 40) {
    return 0;
  }
  if (finePx >= 60) {
...
/** 1 inside the chart, fading to 0 across `fadeZone` px from the nearest edge. */
export const edgeFade = (fromEdge: number, fadeZone: number): number => {
  if (fromEdge >= fadeZone) {
    return 1;
  }
  if (fromEdge <= 0) {
    return 0;
...
/** Ease every tracked label toward its target alpha; drop labels that faded out; seed new ones. */
const settleLabelAlphas = (state: GridState, targets: Map<number, number>, dt: number): void => {
...
export const drawGrid = (
  ...
  const fineTarget = fineLabelTarget(finePx);
  const fadeZone = 32;
  const edgeAlpha = (y: number): number =>
    edgeFade(Math.min(y - pad.top, h - pad.bottom - y), fadeZone);
  ...
  for (let val = first; val <= maxVal; val += fine) {
    const y = toY(val);
    if (y < pad.top - 2 || y > h - pad.bottom + 2) {
      continue;
    }
  ...
  settleLabelAlphas(state, targets, dt);
  ...
  for (const [key, alpha] of state.labels) {
    ...
    const val = key / 1000;
    const y = toY(val);
    if (y < pad.top - 10 || y > h - pad.bottom + 10) {
      continue;
    }
