...
const clampPitch = (v: number) => (v < -1.35 ? -1.35 : Math.min(1.35, v));
// cap thrown momentum to something that still reads as a shape
const clampVel = (v: number) => (v < -6 ? -6 : Math.min(6, v));
...
const levelColor = (q: number): RGBA => {
  const lift = q ** 6 * 0.45;
...
export class HedronRenderable extends Renderable {
  ...
  protected renderSelf(buffer: OptimizedBuffer, deltaTime: number): void {
    ...
    const vertices = this.project(radius, pixelW / 2, pixelH / 2);
    for (const [a, b, c] of FACES) {
      ...
      this.rasterizeFace(pixelW, pixelH, p0, p1, p2);
    }

    this.emitBraille(buffer, w, h, pixelW);
  }

  // Free motion between drags: momentum bleeds off toward the idle state —
  // baseline yaw spin, resting tilt, no pitch drift.
  ...
  // Rotates the mesh by the current yaw/pitch into view space (y up, z toward
  // viewer) and projects it onto the subpixel grid; screen y grows downward.
  private project(radius: number, cx: number, cy: number): ProjectedVertex[] {
    ...
    for (let cellY = 0; cellY < h; cellY += 1) {
      for (let cellX = 0; cellX < w; cellX += 1) {
        ...
        buffer.setCellWithAlphaBlending(
          this.x + cellX,
          this.y + cellY,
          String.fromCodePoint(0x28_00 + dots),
          levelColor(q),
          TRANSPARENT,
        );
      }
