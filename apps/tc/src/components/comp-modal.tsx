...
import { currencyTextFormatProps, staticTextFormatProps } from "@/lib/form-props";
import { cn } from "cn";
...
const Option = ({ children, ...rest }: OptionProps<Company>) => (
...
const Terminology = ({ isoCurrent, rsuCurrent, revenueBased }: ViewProps) => (
  <div className="mt-3">
    {isoCurrent && (
      <div>
        <h2 className="font-bold text-slate-50">Preferred Stock Price</h2>
        <p className="mt-1">
          The preferred stock price is the price at which investors currently pay for shares of the
          company. You can ask your recruiter what the current price is.
        </p>
      </div>
    )}
    {rsuCurrent && (
      <div className="mt-3">
        ...
        <p className="mt-1">This is the stock price at which the company is currently trading</p>
      </div>
    )}
    {revenueBased && (
      <div className="mt-3">
    ...
    <div className="mt-3">
      {isoCurrent && (
        <>
          ...
          <p className="mt-1">
            Depending on the stage of the company expected growth can vary. Investors typically
            expect a 10x return on what they put in.
          </p>
        </>
      )}
      {rsuCurrent && (
        <>
          ...
          <p className="mt-1">
            How much do you expect the stock price to change every year? Anualized growth over the
            last 4 years is a good estimate.
          </p>
        </>
      )}
      {revenueBased && (
        <>
          ...
          <p className="mt-1">
            How much do you expect the company to make every year? Divide this number by the number
            of shares outstanding to get the revenue multiple.
          </p>
          <h2 className="mt-3 font-bold text-slate-50">Revenue Multiple</h2>
          <p className="mt-1">
            The revenue multiple is the ratio of the company’s revenue relative to its stock price.
            You can use your competitors revenue multiple to estimate what your share value would
            be.
          </p>
        </>
      )}
...
interface CompaniesTableProps extends ViewProps {
  companies: Company[];
  onUse: (company: Company) => void;
}
...
export const CompModal = ({
  ...
  const revenueBased = shareCalcType === "revenue";

  const loadOptions = useDebouncedCallback((value: string) => {
  ...
  const loadCompaniesData = (selected: MultiValue<Company>) => {
  ...
  const viewProps = { isoCurrent, revenueBased, rsuCurrent };

  let results;
  if (companiesLoading && !companiesData.length) {
    results = (
    ...
    results = <CompaniesTable {...viewProps} companies={companiesData} onUse={handleUse} />;
  } else {
    results = (
      <div className="flex flex-1 items-center justify-center">
  ...
  return (
    ...
    >
      {view === "terminology" && <Terminology {...viewProps} />}
      {view === "estimate" && (
              ...
              >
                {(fieldProps) => (
                  <Select
                    ...
                    components={{ IndicatorsContainer: () => null, Option }}
                    isMulti
