 ...
 *
 * Importing `auth` pulls in the libsql client, which env.ts refuses to build
 * without a URL, so the test script supplies an unroutable placeholder. Only
 * `auth.options` is read here — nothing opens a connection.
 */
