...
`/api/fal/proxy`, opened on one of the formats in `src/lib/prompt.ts` (sitcom, satirical news, music video, pirate TV, anime news); the programming feeds it one 30s segment per item.
