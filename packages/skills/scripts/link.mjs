...
// Canonical store, read directly by universal agents.
const AGENTS_DIR = path.join(HOME, ".agents");
// Non-universal: symlinks into the canonical store.
const CLAUDE_DIR = path.join(HOME, ".claude");

const DRY = process.argv.includes("--dry-run") || Boolean(process.env.KYH_SKILLS_DRY_RUN);
const FORCE = Boolean(process.env.KYH_SKILLS_FORCE);
...
const symlinksSupported = () => {
  ...
  try {
    ...
    try {
      // Unlink the probe symlink, not its target.
  ...
  try {
    ...
    if (isSymlink(dest)) {
      ...
      if (sameContent(src, dest)) {
        ...
        if (DRY) {
          return log(`would upgrade copy ${rel} -> symlink`);
        }
        fs.rmSync(dest, { force: true, recursive: type === "dir" });
      } else {
        const bak = backupPath(dest);
        if (DRY) {
          return log(`would back up ${rel} -> ${path.basename(bak)} and link`);
        }
    ...
    try {
      ...
      warn(`copied ${rel} (symlink failed: ${error.code ?? error.message})`);
    }
  } catch (error) {
...
const linkClaudeMd = () => {
  ...
  if (fs.existsSync(src)) {
    place(src, path.join(CLAUDE_DIR, "CLAUDE.md"), "file");
  }
};
...
const main = async () => {
  if (process.env.KYH_SKILLS_NO_LINK) {
    return log("KYH_SKILLS_NO_LINK set — skipping.");
  }
  if (process.env.CI) {
    return log("CI detected — skipping link.");
  }
...
try {
  ...
  warn(`unexpected error: ${error?.message ?? error}`);
}
